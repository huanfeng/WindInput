...
fn bench(label: &str, cc: &CommonChars, texts: &[&str], rounds: usize) {
    // 预热，避免首轮的分支预测/缓存冷启动混进读数。
    let mut sink = 0usize;
    for t in texts {
        sink += cc.is_string_common(t) as usize;
    }
    let t0 = Instant::now();
    for _ in 0..rounds {
        for t in texts {
...
fn bench_is_string_common_hot_path() {
    ...
    bench("无覆盖（闸门关）", &plain, &texts, rounds);
    ...
    bench("200 条单码位覆盖（闸门关）", &single, &texts, rounds);
    ...
    bench("+1 条多码位覆盖（闸门开）", &multi, &texts, rounds);
}
