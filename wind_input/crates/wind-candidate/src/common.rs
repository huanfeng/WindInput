...
/// 候选文本看起来是不是**字词**——首字符落在 [`is_common_scope`] 里（汉字或 PUA 字形），
/// 而不是符号、表情、标点或西文。
///
/// # 为什么需要它
///
/// 码表的二简位不一定都排着字词：「部分二简编码无字词可编，也编成了符号」（论坛 t11）。
/// 逆切分把两段拼起来时，若某段只能查到 `→` / emoji / 标点，拼出来的是「字 + 符号」这种
/// 读不通的东西 —— 该段应当**视同空码**，而不是照拼。
///
/// # 判据只看首字符
///
/// 够用且不误杀：符号库的条目整体就是符号，不会「首字符是符号、后面跟着汉字」；而
/// 「AA 制」「3D」这类西文起头的词条本就不适合参与二简组词。
///
/// ⚠️ **滤不掉字根**：`亻` `辶` 这类部件的码位就在基本汉字块里，与真汉字无从区分。
/// 想把它们也挡住，得靠词库侧不把字根编进二简位，或者另立一份字根表 —— 不在本函数职责内。
pub fn is_word_like(text: &str) -> bool {
    text.chars().next().is_some_and(is_common_scope)
}
