...
// 守卫已挪到 wind-utils/tests/line_ending_coverage.rs：原先这里那版按「参数叫 text
// + 零缩进 pub fn」扫描，漏掉了 impl 块里的全部关联方法（整机备份恢复的四个 JSONL
// 入口就是这么被放过的），判据也不对。新版按「函数体里有 .lines()」扫全 workspace。
