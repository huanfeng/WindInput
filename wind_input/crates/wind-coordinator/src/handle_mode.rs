...
/// 只是死键，同样归字面。`.` 这类「本透镜合法、却换不来任何候选」的字符只能靠它——它自身
/// 永不越界，上面那条判据对它无能为力。见③处的 ★。
///
/// # 为什么是缓冲的纯函数，而不是一个状态位
///
        ...
        match data.key_code {
            ...
            _ => {
                ...
                if lens != MixLens::Free {
                    ...
                    //
                    // 一条候选都没有，这个键就不可能在表达选词意图，它只是个死键；而⑤的判据
                    // 「既不是编码也不是功能键 → 只能是字面」正要求功能键是**真的在起作用**。
                    //
                    // 不补这道判定，`auto` 的越界推导在数字透镜下有一整类输入永远够不着：
                    // 越界字符要先进缓冲才能把透镜带进 `Free`，而字母在③被无条件吃掉，于是
                    // 它永远进不来。`.` 正落在这个死角——它**是**表达式字符（`is_expr_char`），
                    // 缓冲 `.` 自身永不越界，可三个内置来源对光秃秃一个点一条候选也给不出
                    // （`is_decimal_number` 要求首字节是数字，`.5` 同样为空；calc 还要求首字符是
                    // 数字/括号/正负号）。用户想打 `.txt`，实得组合区永远停在 `.`（A2-23 / t157）。
                    // 大写字母早有出路（①的 `free_on && shift` 臂），小写却没有——`.TXT` 打得出、
                    // `.txt` 打不出，这个不对称本身就是漏了本判定的证据。
                    //
                    // ⚠️ 判据刻意是「一条都没有」而不是「第 off 条不存在」：后者会让瞄歪的选词
                    // （候选只有 3 条却按了第 4 个选词键）直接变成字面输入，把整个转换会话拖进
                    // `Free`——`;nihao` 误按 `4` 得到字面 `nihao4`。空列表则没有这种误读空间。
                    //
                    // ④（配置的二三候选键）**刻意不加同样的判定**：它只在
                    // `free_input_takes_select_keys = false` 时可达，那是用户明写的「这两个键
                    // 归选词」，空候选下让它们改作字面等于推翻该配置。
                    if let Some(off) = sel
