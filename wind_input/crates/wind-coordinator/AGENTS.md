...
| `src/handle_aux_code.rs` | 辅助码模式：进入/退出/筛选/导航/共享键（翻页+辅助码同一键）/自动退出。`init_aux_overlay` 不发 UI 通知（由调用方统一发送），`enter_aux_code_from_page` 保存/恢复 `current_page` 以保留翻页结果 |
...
- **辅助码与翻页共键**：同一按键可同时绑定翻页（`session_keys`）和辅助码触发（`[key_actions]`），系统运行时自动识别。检测方法 `is_shared_page_aux_key` 在 `handle_aux_code.rs` 内，仅做 VK 比较。集成点在 `message_handler.rs` 的 `apply_session_action` 之后：翻页成功后调用 `enter_aux_code_from_page` 尝试进入辅助码。辅助码模式内，`handle_candidate_nav_or_auto_exit` 直接调用 `page_prev`/`page_next`（绕过 `handle_candidate_nav` 的 `include_printable=false` 过滤），并在从非首页翻回首页且 session 为空时自动退出。`enter_aux_code` / `enter_aux_code_from_page` 有防重入守卫（`state.active == Some(ModeKind::AuxCode)` 时返回 None）。`init_aux_overlay` 不发送 UI 通知（`notify_ui_update` 由调用方统一发送），`enter_aux_code_from_page` 在 `init_aux_overlay` 前后保存/恢复 `current_page`，确保共享键路径只发两次 UI 更新（避免三重通知闪烁）。所有辅助码逻辑内聚在 `handle_aux_code.rs` 内，其他模块只提供信息 pathway。
