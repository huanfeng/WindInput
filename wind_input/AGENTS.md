...
- 三层配置加载：代码默认值 → `data/config.toml`（系统预置）→ `%APPDATA%/WindInput/config.toml`（用户配置）

### 枚举常量规范（强制）
**红线**：有限取值字符串必须通过 `type Foo string` + `const` 块定义在 SSOT 文件（`pkg/config/enums.go`、`pkg/keys/`、`internal/schema/types.go`），并用具名类型贯穿到调用点；禁止 `iota` 整数枚举；新增类型须加 YAML round-trip 测试。
完整规则、Go 样板、按键 token / 组合键群处理方式、PR 自检命令见 [`/docs/design/enum-constraint.md`](../docs/design/enum-constraint.md)。
