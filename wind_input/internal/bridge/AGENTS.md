...
平台特定能力:
- **Win**: Host Render（**单块全局命名共享内存** + **per-实例(clientID) 命名 Event** 把候选词位图传给白名单宿主进程，绕过 Win11 开始菜单 Band 层级；SHM 共享省内存，event 按 bridge 连接隔离——同 PID 多实例（如两个记事本窗口）各有独立 event，配合 SHM 头 `TargetInstanceID` 让目标实例渲染、兄弟实例隐藏）
...
| `host_render.go` | `HostRenderManager`: 白名单进程的宿主渲染状态; 通过 `OpenProcess`/`QueryFullProcessImageNameW` 识别进程名称。**单块全局 SHM + per-实例 event 模型**: 每窗口种类全局一份 `SharedMemory`（懒建常驻，`winSHMNameFor`，物理页跨进程共享→每类内存恒一份），每个 **bridge 连接(clientID)** 每类一个私有 `NamedEvent`（`winEvtNameFor(clientID,kind)`=`Local\WindInput_EVT_C<clientID>`+后缀）存于 `clients` map（键为 clientID）。`HostRenderState.InstanceID`=clientID。白名单支持 `filepath.Match` 通配符（`*` 短路匹配全部→全局模式）。`WriteFrameForKind(kind,pid,target,...)`/`WriteHideForKind(kind,pid)` 写共享 SHM（帧盖 `TargetInstanceID=target`）后 signal **该 PID 下所有实例**的 event——目标实例渲染、兄弟实例据 target 不符自隐藏（`eventsForLocked` 收集）。`HasChannel(clientID,kind)` 供 `GetActiveHostRenderFor` 判活动实例是否可宿主。`SetupSeq` 防竞态保留；`CleanupClient(clientID,seq)`/`GetSetupSeq(clientID)` 按 clientID；`CleanupAll` 关所有 event + 全局 SHM |
| `shared_memory.go` | `SharedMemory`: 纯命名共享内存（**不含 event**，`WriteFrame`/`WriteHide` 不 signal，由调用方 signal）; `WriteFrame(img,x,y,rects,renderedHover,targetInstanceID)` 末参把目标实例 ID 写入头部 [52:56]; `NamedEvent`: per-实例 auto-reset 唤醒 event; `hostRenderSecurityAttributes` 共用 AppContainer 低完整性 SDDL (`S:(ML;;NW;;;LW)`) 给 SHM + event; `WriteFrame` RGBA→BGRA 转换写入 |
...
- **Host Render 流程（多窗口）**：C++ DLL 看到 `StatusHostRenderAvail` 标志后发送 `CmdHostRenderRequest`；Go 侧 `HostRenderManager.SetupHostRender(clientID, pid)` 对 `hostRenderKinds`（候选 + tooltip + 状态窗）**每类懒建一块全局 SHM + 为该实例每类建私有 event**，返回 `CmdHostRenderSetup` 响应（顶层 `instanceID`(=clientID) + `[]ipc.HostRenderSetupEntry`，每条 {kind, maxBufSize, shmName, evtName}），DLL 每条目建一个 band 窗口并记下 `instanceID`。活动实例的各窗口更新经 `HostRenderManager.WriteFrameForKind(kind,pid,target,...)`（写该 kind 的 SHM + 盖 `TargetInstanceID` + signal 该 PID **所有实例**的 event）；`server.GetActiveHostRenderFor(kind)` 用 `activeInstanceID`（=活动连接 clientID，在 `processRequest` 顶部随 key/focus/activate 命令更新）作 target 返回 (writeFrame, hideFunc) 闭包，候选用 `GetActiveHostRender()`（= kind 候选）
- **为何每类独立通道 + 为何 event 按实例隔离**：候选/tooltip/状态窗**可能同时可见**，不能共用单 bitmap 通道（会互相覆盖）→ 每类一块 SHM。共享内存命名：`Local\WindInput_SHM`（候选，向后兼容）/ `_TIP`(tooltip) / `_STS`(状态)（+变体后缀，每类全局唯一，物理页跨进程共享→每类内存恒一份）；事件命名：`Local\WindInput_EVT_C<clientID>` + 同后缀（**按 (clientID,kind) 隔离**）。**关键教训**：event 必须 per-实例，绝不能 per-PID——同一进程多个 TextService 实例（如两个记事本窗口=同 PID）的渲染线程若等在同一个 auto-reset event 上，`SetEvent` 只唤醒其中一个（不确定是谁）→"两层候选、只隐一个"。修复=每实例独立 event（Go signal 该 PID 全部实例）+ SHM 头 `TargetInstanceID`（DLL 渲染线程仅当 `header.targetInstanceId==_instanceId` 才渲染，否则隐藏自己）。单块 SHM 不变，靠 target 字段区分目标，无竞态
...
- **tooltip host render（Phase 1）**：高 Band 宿主下 tooltip 也走 host render（否则被候选 band 窗口遮挡）。`Coordinator.updateHostRenderState` 除候选外，再取 `GetActiveHostRenderFor(HostWindowTooltip)` 包成 `func(img,x,y)`（rects=nil/renderedHover=-1）经 `Manager.SetTooltipHostRenderFunc` 注入 `TooltipWindow`；后者 `Show/Hide/ForceHide` 内部分支：host 模式渲染 bitmap 写 tooltip SHM、本地窗口隐藏（`hostVisible` 镜像）。dispatch（`CmdTooltipShow`→ForceHide+Show / `CmdTooltipHide`→ForceHide）无需改动。状态窗同理：`GetActiveHostRenderFor(HostWindowStatus)`→`Manager.SetStatusHostRenderFunc`→`StatusWindow.SetHostRenderFunc`，host 逻辑集中在 `StatusWindow.Show/Hide` 内分流（`doShowStatus`/`doHideStatus`/`scheduleHide` 统一走 `Show`/`Hide`）。tooltip/状态均**纯显示**（无鼠标交互，host 模式下失去拖动/右键菜单）
