...
- `CmdHostRenderFailed`（上行 0x0212, async）Win host render 专用：DLL 建 band 窗口失败（受限 UWP 宿主如微软商店，`CreateWindowInBand` 即便回退 band=0 仍失败），候选回退本地窗口。payload=reason u32（`HostRenderFailWindowCreate=1`）。bridge `handleHostRenderFailed` 经可选接口 `hostRenderFailureReporter` 派发到 `Coordinator.HandleHostRenderFailed(pid,reason)`：按 PID 去重记 WARN + 弹一次 toast 告知用户
