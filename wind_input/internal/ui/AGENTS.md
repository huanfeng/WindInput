...
| `viewbox_paint.go` | 盒模型绘制层：`PaintTree` 三趟遍历（形状/背景图/z<0 → 文本 → z>0 覆盖图），复用 `theme.DrawBackground` + 注入 `TextDrawer`；含 chevron 箭头绘制。**渐变**（P7-E）：`paintShapes` 在底色之后、背景图之前插入 `theme.RasterizeGradient`（按 View rect 栅格化）经同一 `DrawBackground` 圆角路径绘制 |
