...
| `viewbox.go` | **盒模型 View 渲染引擎**(P1，设计见 docs/design/archive/theme-view-architecture.md)：`View`/`Fill`(底色+渐变+背景图依次叠加，`Gradient *theme.RVGradient` paint 时栅格化)/`Border`/`ImageLayer`(图片或纯色层)/`TextStyle`/`Shadow`/`GlyphKind`(矢量箭头)/`Edges` 类型 + measure/arrange（行/列流式、margin/padding、交叉轴对齐、`Stretch` 撑满、`Grow` 弹性占位右对齐）；布局层经 `TextMeasurer` 注入文本度量，可纯断言单测 |
| `viewbox_paint.go` | 盒模型绘制层：`PaintTree` 三趟遍历（形状/背景图/z<0 → 文本 → z>0 覆盖图），复用 `theme.DrawBackground` + 注入 `TextDrawer`；含 chevron 箭头绘制。**渐变**（P7-E）：`paintShapes` 在底色之后、背景图之前插入 `theme.RasterizeGradient`（按 View rect 栅格化）经同一 `DrawBackground` 圆角路径绘制。**背景图/层定位 + 裁剪**：`drawLayer` 把 layer 按 `anchor`+`OffsetX/Y`(dp)+`OffsetXPct/YPct`(百分比，相对 host 宽/高于此换算)定位，经 `theme.DrawBackgroundClipped(...,host)` 矩形硬裁到 host 不外溢；`paintShapes` 中 `Fill.Positioned`(配了 anchor/offset/size)的背景图复用 `drawLayer` 通路，未配则全覆盖老路（零回归） |
| `viewbox_image_resolver.go` | **共享位图/矢量图基础设施（P8 切片6）**：`imageResolver`（解码缓存 + mutex 并发安全）的 `imageForRef`/`fillFor`/`appendLayers`，从 `renderer.go` 原 P7-C 实现抽出；候选窗 `Renderer`（委派）与 status/tooltip/menu/toast 各渲染器共享；resources 表（ref→path/dataURI）按帧从各自 `resolvedV3.Resources` 传入，换主题 `reset()` 清缓存。**矢量+染色**：`resolveImage(ref, resources, w, h, tint)` 统一入口——SVG ref 经 `theme.RasterizeSVG` 按目标 w×h 现场栅格化（缓存键含尺寸；动态背景 w/h<=0 兜底 64²）、位图走 ref 缓存；`tint` 非 nil 经 `tintMask`（把图当 alpha mask 用主题色填充，缓存键含色值）。`fillFor`/`appendLayers` 已改走 `resolveImage`（背景/层支持 SVG + tint）。**定位 + 百分比 offset**：`fillFor(...,scale)` 对配了 anchor/offset/size 的背景图标 `Fill.Positioned` 并把 offset/size 的 dp 部分 ×scale、百分比透传（`bgPositioned` 判定）；`appendLayers` 把层 offset dp 部分 ×sc、百分比(`OffsetXPct/YPct`)透传（paint 阶段相对 host 换算）。`fillFor` 现需 `scale` 实参——候选窗经 `Renderer.fillFor`、其它窗口直接 `ir.fillFor` |
...
| `viewbox_build_vertical.go` | `buildVerticalCandidateTree`（竖排：每候选一行全宽、翻页区底部居中，同从 `r.resolvedViews` 取外观）+ 横竖共用的 `buildPager`（chevron + 页码 + 命中按钮）。**翻页可配**：页码/启用态箭头色取 `views.footer_bar.color`（未配零回归）；箭头图 `views.footer_bar.prev_image`/`next_image`（`fb.PrevImage`/`NextImage`，SVG/PNG 经 `imgRes.resolveImage` 按图标尺寸栅格化、可 tint，居中绘制；解码失败回退内置矢量 chevron）。**翻页箭头左右 padding** 复用 `views.footer_bar.padding`（`fb.PadLeft/PadRight`，未配兜底 6，默认逐像素零回归）。**横竖一致**：`item.padding` 四向在竖排也生效（左 padding 经 `itemPadLeft`，有 accent 时让位给 rail，`effLeft=itemPadLeft+railW` 横竖统一；横排 rail 宽同加 `minW` 保护）；候选项间距分离为**横向 `gap`**（横排 `boxGap`）/ **纵向 `row_gap`**（竖排 `LayoutColumn` Gap，默认 0=紧贴） |
...
| `viewbox_tooltip.go` | **Tooltip View 化（P4-B）**：`buildTooltipTree`（多行 LayoutColumn + `\t` 列对齐 LayoutRow，每列 `FixedW` 取最大宽对齐、缺列补空占位 cell；行数上限 20 / 单列截断 / 多列末列截断逻辑预处理）+ `(*TooltipWindow).resolveTooltipColors`（views.tooltip token > `Palette.Tooltip` > 默认）。`tooltip.go:render` 改走此路径 + `newSharedDrawContext`（取代旧 gg 直绘 + `getTooltipColors`）；`truncateLineToWidth` 形参改 `TextMeasurer` 以可单测。**间距可配**：行距/列距读 `views.tooltip` 的 `line_spacing`/`col_gap`（`node.LineSpacing`/`ColGap`，未配兜底 2/16，零回归） |
...
| `viewbox_menu.go` | **菜单 View 化（P4-D）**：`buildMenuTree`（root LayoutColumn + 每项 LayoutRow：check/text(Grow)/arrow，勾选✓/箭头▸/文本走 View 文本叶子，hover 满宽背景，分隔项收集供后处理画线）返回 `menuTree`；`(*PopupMenu).resolveMenuColors`（7 色映射 `Palette.PopupMenu`，views.menu token 覆盖含 hover 状态）。几何 hardcode×scale 与命中测试一致；**padding 独立化**：root 消费 `views.menu.root.padding` 左右、item 上下 padding 独立生效（`FixedH=itemH+padT+padB`，仿候选项，对称 padding/未配=零回归） |
...
| `toast_renderer.go` | `ToastRenderer`：toast 图像渲染（标题 + 多行正文 + 圆角矩形 + Level accent 边框），复用 `TextBackendManager`（DirectWrite）。**间距可配**：行距/标题距读 `views.toast` 的 `line_spacing`/`title_gap`（`node.LineSpacing`/`TitleGap`，未配兜底 4/6）；accent 条宽度/内缩为视觉签名常量仍 hardcode |
...
- `CandidateCallback.OnDragEnd(x, y)` 在候选窗拖动结束时回调，coordinator 用于将位置持久化到 `state.toml`（仅当当前应用启用了 pin 规则时落盘）
