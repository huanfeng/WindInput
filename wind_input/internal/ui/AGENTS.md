...
| `viewbox_image_resolver.go` | **共享位图基础设施（P8 切片6）**：`imageResolver`（解码缓存 + mutex 并发安全）的 `imageForRef`/`fillFor`/`appendLayers`，从 `renderer.go` 原 P7-C 实现抽出；候选窗 `Renderer`（委派，零行为变化）与 status/tooltip/menu/toast 各渲染器共享同型基础设施；resources 表（ref→path/dataURI）按帧从各自 `resolvedV25.Resources` 传入（不持快照），换主题 `reset()` 清缓存 |
