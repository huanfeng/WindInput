...
| `Sources/WindInputKit/IPC/BinaryCodec.swift` | 帧 encode/decode (镜像 Go `internal/ipc/binary_codec.go`)。`encodeFocusGainedFrame(inputScopeMask:)`: FocusGained 帧布局 `pid:u32(0占位) + inputScopeMask:u64` (bit31=IS_PASSWORD 标记密码框, server_darwin 从 `payload[4:12]` 解析; 旧版空帧=mask 0) |
