...
| `Sources/WindInputApp/UI/CandidatePanelHost.swift` | 候选框承载层: 订阅 push, 收 CmdHostRenderFrame→SHM→NSPanel, CmdCandidateRects→hit-test, CmdModeStatus→ModeStatusController; 鼠标选词回发。导出 `unifiedMenuItems()`/`sendMenuAction(_:)` 供三处菜单 (候选框右键/菜单栏指示器/系统输入菜单) 复用同一 IPC 请求与回发路径 |
