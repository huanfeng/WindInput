...
| `app.sh` | macOS IMKit `.app` 打包：`swift build --product wind-input-app` + 拼 `Contents/{MacOS,Resources/lproj,Info.plist,PkgInfo}` + 拷 `AppIcon.icns`（应用图标，plist `CFBundleIconFile`）+ `codesign --options runtime`（默认 ad-hoc；`SIGN_IDENTITY="cert name"` 切真证书）。输出 `wind_macos/build/WindInput.app` |
...
| `pkg.sh` | 把**三件套**（IME + Go 服务 + 设置 app）打成单个 **`.pkg` 安装器**（面向终端用户分发）：payload 把三件套 + 三个 `deploy/install_*.sh` 铺到系统暂存 `/Library/Application Support/WindInputInstaller/`，`pkgbuild --scripts` 挂 `pkg_resources/postinstall`，输出 `wind_macos/dist/WindInput-<版本>.pkg`。`--build` 先构建全部组件。安装 `sudo installer -pkg ... -target /` 或双击。**未公证版**首启需绕过 Gatekeeper，Tahoe 系统设置 UI 硬墙需公证才解 |
