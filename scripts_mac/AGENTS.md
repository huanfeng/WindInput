...
`--debug` 让 `build/{app,build,setting}.sh` + `deploy/install_{app,service,setting}.sh` + `vm/deploy.sh` 全链路产出 **debug 变体**, 与 release **作为两个独立输入法/服务/设置应用共存** (日常用正式版、旁边迭代测开发版, 互不干扰)。变体差异 (与 Go `buildvariant` 对齐):

| | release | debug |
|---|---|---|
| `.app` / bundleID | `WindInput.app` / `to.feng.inputmethod.WindInput` | `WindInputDebug.app` / `…WindInputDebug` |
| 服务 LaunchAgent | `to.feng.windinput.service` | `…service.debug` |
| 运行时目录 (socket/config) | `~/Library/Application Support/WindInput` | `…/WindInput_debug` |
| SHM 段 | `/WindInput_SHM` | `/WindInput_SHM_debug` |
| 设置应用 | `清风输入法设置.app` / `com.wails.wind_setting` | `清风输入法设置开发版.app` / `…wind_setting_debug` |

关键实现点: `app.sh --debug` 用 sed 把 Info.plist 的 bundleID 串整体替换 (一并改连接名/mode-id) + 显示名加「开发版」, 可执行名恒 `WindInput` (仅 .app 目录名变, 故 install/uninstall **按 .app 路径杀进程**, 不能按进程名——两变体进程同名会误杀)。`build.sh` 非 universal 分支也锁 `CGO_ENABLED=0` (cgo 版服务启动期死锁)。Swift `.app` 从自身 bundleID 后缀 (`Bundle.main.bundleIdentifier` 末尾 `Debug`) 推导运行时/SHM 后缀。设置应用经 `buildvariant` ldflag 自动连各自 `rpc.sock` (配置分离)。
...
| File | Description |
|------|-------------|
| `build.sh` | macOS 端 Go 服务构建（对位 `build_all.ps1` 的可裁剪子集）：下载 rime-frost / rime-wubi / OpenCC 词库源到 `.cache/`，跑 `gen_unigram` + `dictgen` + `gen_opencc_dict`，产出 `build/{wind_input, data/}`（debug variant 产出 `build_debug/{wind_input_debug, data/}`）。子命令：`all` / `service` / `data` / `clean`，开关：`--debug` / `--universal`（arm64+x86_64，`CGO_ENABLED=0` 双架构交叉编译 + `lipo` 合并；亦可 `WIND_MAC_UNIVERSAL=1` 环境变量统一开关） |
| `app.sh` | macOS IMKit `.app` 打包：`swift build --product wind-input-app` + 拼 `Contents/{MacOS,Resources/lproj,Info.plist,PkgInfo}` + 拷 `AppIcon.icns`（应用图标，plist `CFBundleIconFile`）+ `codesign --options runtime`（默认 ad-hoc；`SIGN_IDENTITY="cert name"` 切真证书）。**版本贯通**：若仓库根有 `VERSION` 文件则注入 `CFBundleShortVersionString`/`CFBundleVersion`（CI 由 tag 写入，pkg.sh 据此取版本）。开关：`--debug` / `--no-sign` / `--universal`（`--arch arm64 --arch x86_64`，产物落 `.build/apple/Products/<Config>/`；或 `WIND_MAC_UNIVERSAL=1`）。输出 `wind_macos/build/WindInput.app` |
| `setting.sh` | 在 macOS 上构建 `wind_setting.app`（Wails+Vue 设置界面）：容忍 pnpm 11 ignored-builds 退出码、用 `vite build` 跳过 vue-tsc 门禁、把 `build/data` 拷进 `.app`。需 `build/build.sh data` 先产出词库。开关：`--universal`（`wails build -platform darwin/universal`，默认 `darwin/arm64`；或 `WIND_MAC_UNIVERSAL=1`）。输出 `wind_setting/build/bin/wind_setting.app` |
| `pkg.sh` | 把**三件套**（IME + Go 服务 + 设置 app）打成单个 **`.pkg` 安装器**（面向终端用户分发）：payload 把三件套 + 三个 `deploy/install_*.sh` 铺到系统暂存 `/Library/Application Support/WindInputInstaller/`，`pkgbuild --scripts` 挂 `pkg_resources/postinstall`，输出 `wind_macos/dist/WindInput-<版本>-macOS.pkg`（`-macOS` 后缀与 Windows `-Setup.exe`/`-Portable.zip` 在同一 Release 区分）。`--build` 先构建全部组件，`--universal`（`export WIND_MAC_UNIVERSAL=1` 下传子脚本 + distribution.xml `hostArchitectures="arm64,x86_64"`，与产物架构一致）。**公证预留**：配齐 `MACOS_DEVELOPER_ID_INSTALLER` + `MACOS_NOTARY_{APPLE_ID,PASSWORD,TEAM_ID}` 环境变量则 productbuild 后自动 `productsign` + `notarytool submit --wait` + `stapler staple`，否则保持 ad-hoc。安装 `sudo installer -pkg ... -target /` 或双击。**未公证版**首启需绕过 Gatekeeper，Tahoe 系统设置 UI 硬墙需公证才解 |
| `pkg_resources/postinstall` | `pkg.sh` 的 `pkgbuild --scripts` postinstall：installer 以 root 跑，但 IME/TIS/LaunchAgent 必须以登录 GUI 用户身份在其会话里装 → 用 `launchctl asuser $UID sudo -H -u $USER` 双重下沉，依次调 `install_service.sh --from`/`install_setting.sh --from`/`install_app.sh --from`；再把脚本拷到 `~/Library/Application Support/WindInput/installer/` 并在 `~/Applications/` 放「卸载清风输入法.command」；最后清系统暂存。需登录 console 用户（headless `installer` 亦然） |

### deploy/

| File | Description |
|------|-------------|
| `install_app.sh` | 把 `WindInput.app` 装到 per-user `~/Library/Input Methods/`（用户域，**不要 sudo**）：cp + 对 ad-hoc 产物原地去 hardened-runtime 重签（`codesign --force -s -` → 纯 ad-hoc `flags=0x2`，与 Fcitx5 一致；真证书产物则保留不降级）。**`SIGN_IDENTITY` 非空时改用固定证书重签**（`codesign --force --sign "$SIGN_IDENTITY"`，TCC 授权基于证书 csreq、不随 cdhash 失效 → 重部署不丢辅助功能授权；无头 ssh 部署还需 `SIGN_KEYCHAIN_PW` 先解锁 login keychain 供 codesign 访问私钥）。然后 `lsregister -f -R` 刷 LS DB + `WindInput --register-input-source` 让 IME 自身进程调 TIS API。`--uninstall` 完整清理（删 .app + 清 HIToolbox plist + 缓存 + SIGTERM 重启 input agents），`--build` 先 build。**不再 `spctl --add`**（Tahoe 已移除该能力且证伪无必要）。能否进系统设置「可添加列表」取决于 `Info.plist` **不带** `tsInputModeDefaultStateKey`（带则 mode 注册即「已启用」却不落盘 → 被「+」列表过滤掉看不见，见 `wind_macos` Info.plist 说明） |
| `install_service.sh` | 把 Go 服务（`wind_input` + `data/`）装到 per-user `~/Library/Application Support/WindInput/service/`，生成并装 LaunchAgent `to.feng.windinput.service.plist`（`RunAtLoad`+`KeepAlive` 开机自启），`launchctl bootstrap/enable/kickstart` 启动，末尾验证 push socket + err.log。普通用户运行（不要 sudo）。开关：`--debug` / `--from <dir>`（指定源目录，部署脚本远程用）/ `--uninstall`。装时 cp 后 `codesign --force -s -` 原地 ad-hoc 重签（跨机同路径 redeploy 触发 amfi cdhash 缓存失配 → `OS_REASON_CODESIGNING`；`SIGN_IDENTITY` 非空时改用固定证书重签，与 install_app.sh 同理，`SIGN_KEYCHAIN_PW` 解锁 keychain）+ `pkill -f "$INSTALL_ROOT/wind_input"` 清孤儿进程（bootout 只杀托管实例）。服务用 `exeDir/data` 定位词库，故二进制与 `data/` 必须同目录 |
...
| `setup_signing.sh` | 命令行创建本机 self-signed Code Signing 证书：openssl 生成 RSA + X509（codeSigning EKU）→ PKCS12（OpenSSL 3.x 加 `-legacy`，**LibreSSL 自动省略**以兼容系统自带 openssl）→ import 到 login keychain → `add-trusted-cert` 加 trust。子命令: 默认创建 / `check` / `remove` / **`grant`**（对已建证书设 `set-key-partition-list`，授权 codesign 非交互访问私钥——`import -A` 仍不够，无头 ssh 部署 codesign 否则 `errSecInternalComponent`）。清理走 `purge_cert`（login + System keychain，System 需 sudo，带 20 次上限防 delete 删不掉时死循环）|
...
- `deploy/setup_signing.sh` 需要 openssl（OpenSSL 3.x 用 `-legacy`，系统自带 LibreSSL 已兼容——脚本按版本自动省略该标志）+ 用户 login keychain 已解锁（GUI 会话）；无头 ssh 部署还需先跑 `setup_signing.sh grant` 设 partition-list
