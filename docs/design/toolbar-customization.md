...
⚠️ **「老用户外观不变」只对停留在出厂默认的人成立**。在设置页动过工具栏条目的用户，
其 L3 里那份 `items` 是显式写出的、含裸 `"s2t"`——他们会看到这一格从此常驻。
那**正是他们勾选时期待的行为**（此前只在简繁开着时出现才是缺陷），所以不做静默改写：
把别人明确勾上的一格自动改成 `-s2t`，是拿一种意外换另一种。同样地，他们也不会自动
拿到 `soft_keyboard` 格——出厂默认的变更本就不该翻越用户的显式配置。
...
### 13.6 审查抓出的四条（都编译通过、测试全绿）

**① 收口点上游有个早返回，绕过了收口。** `MenuCmd::SoftKeyboardPage` 改成「开面板 →
`ui_softkeyboard_page`」之后，我把开面板那步的推送删了，理由是「收口在下面那句里」。
但 `ui_softkeyboard_page` 在**下标越界时只 `warn!` 就返回**，走不到收口——而菜单是照
构建时的面表列的，配置一重载就可能少一面。那时面板已经开着并接管按键，C++ 却收不到
状态位、图标也不亮，正是这组提交要消灭的那对症状。

★ 判据：**谁改了状态谁负责收口**。「下游会兜」只在下游无条件执行时才成立，
而下游的早返回不归你管。

**② 单测写了进程外的全局状态。** `save_softkeyboard_page` → `Config::state_dir()` 是
`%LOCALAPPDATA%\WindInput[Dev]`，单测构造的协调器照样走得到：跑一次 `cargo test` 就改掉
开发者本机的 `last_softkeyboard_page`，还会与**正在运行的服务**抢同一个 `state.toml`
（两边都是 load-modify-save，一次丢更新就吞掉刚存好的 `toolbar_positions`）。

⚠️ 第一版的注释只防住了「读到被污染的镜像」，**没防住写**——那是把症状当成了病。
处置：判据（`softkeyboard_page_to_persist`）与写盘分两层，写盘那层加一道 `store.is_none()`
的门。判据借 `store` 是因为它的文档本来就写着「None = 无持久化（headless 测试）」，
这个语义不是新赋予的；而 Android 生产形态有 store，门恰好只挡测试夹具。

**③ 去重镜像在写盘失败时也更新了。** 一次偶发写失败（文件被同步/备份工具锁住、磁盘满）
就让镜像谎称「已经存过了」，此后同一面的每次关闭都被去重挡掉——本次会话再也存不进去，
线索只有一行 `warn`。★ **缓存要在成功之后才更新**，这是缓存与副作用配对时的通例。

**④ 分格右键把判据③削弱了。** §2.2 判据③是「隐藏齿轮不锁死用户——右键工具栏任意位置
同样弹主菜单」。分格右键之后，6/8 的格给的是精简菜单，隐藏了齿轮的用户只剩 **12dp 的
拖动柄**通向主菜单——一个要瞄准的目标。

⚠️ **守门测试证不了这件事**：`every_toolbar_cell_either_customizes_or_falls_back` 断言的
恰恰是「这些格有自己的菜单」，与判据③方向相反。⇒ 每份分格菜单末尾挂一条
`MenuCmd::OpenMainMenu`，并另立一条方向相反的守门。**加功能时顺手削弱了某条旧不变量，
旧不变量的测试往往正是那个不会红的**。

⑤ 另有两处「测试对它要钉的性质是盲的」：分格菜单三个开关的顺序与主菜单不一致，而断言
按 label 查找、换成任意排列都绿；纵条锚点的贴格没生效（`Side` 读的是 `bottom`，我只传了
`y`），而代码看着是对的。两处都补了直接断言那个性质的测试。
