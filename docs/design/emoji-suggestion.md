...
| `tail` | 插在**最后一条常用候选之后**（`emoji_tail_insert_at`） | 原有序号不漂 |
...
★ **`tail` 不是物理末尾**（2026-09-07 真机修正）。初版直接 `extend` 到列表尾，智能档下
被排到生僻字之后：智能档只滤掉「同码位有常用字」的生僻字，孤儿码位的生僻字照常留在
尾部，emoji 落在它们后面等于要翻好几页，形同没有。现在的落点判据与过滤器共用
`wind_candidate::is_common_like`：最后一条常用候选（常用字 / 短语 / 命令 / 分组）之后、
放宽补充段之前；全无常用候选时落到补充段之前的末尾，不抢首位。

⚠️ **`comment` 档暂时从设置页隐藏**（2026-09-07 真机结论）：注释区目前没有上屏入口，
选了它 emoji 只能看不能选，与「什么都没发生」无异。core 侧取值与 `${emoji}` 变量原样保留，
设置仓 `NARROWED_ON_PURPOSE` 登记了这一项；待注释上屏功能落地后，把选项加回
`settings_manifest.toml` 即可。副作用：用户在配置文件里手写 `comment`，打开设置页保存
一次会被改回 `after`——这是隐藏取值的已知代价，功能本身处于半成品状态时可以接受。
...
- **右键菜单对 emoji 候选五项灰显、写端拒绝**：置顶 / 前移 / 后移 / 隐藏 / 恢复默认都
  走 shadow 或词库落点，而 emoji 候选 `code` 恒空、没有落点，规则写进去读端永远命中
  不了。菜单侧 `handle_menu.rs` 与写端 `candidate_op` 各有一道 `is_emoji_suggestion`
  守卫，**放开时必须两端一起**。「设为常用字 / 生僻字」与「复制」不受影响。可调整性的
  方向见 §9。
...
> ★ 本节的方向已在 [`text-mapping-tables.md`](text-mapping-tables.md) 展开为完整设计：
> 内置符号表、多表框架、编辑入口，以及「还能用映射表做什么」的判据与取舍。
> 那份文档同时记录了一个由此暴露的**现存缺陷**（U+FE0F 未归一导致同一符号重复出候选）。


用户对 emoji 候选可能想调的只有三件事，都不是 shadow 的语义：

| 诉求 | 例子 | 落点 |
|---|---|---|
| 换顺序 | 「开心」下想让 😊 排在 😄 前 | 按 `(词, emoji)` 记序，只在该词的 emoji 列表内生效 |
| 不要某个 | 「一个人」不想出 👤 | 按 `(词, emoji)` 记隐藏 |
| 加自己的 | 「猫猫」→ 🐱 | 用户自定义词表 |

**建议模型**：

1. **用户词表**：`<userdata>/emoji/emoji_user.txt`，与上游同格式（`词<TAB>词 emoji…`），
   建缓存时作为**第一张表**参与合并（同键合并保序 ⇒ 用户的排前面），并进指纹
   （`fp_sources`）。这一步复用现有 `load_or_build`，零新格式。「加自己的」与「换顺序」
   都能靠它表达——用户把自己想要的顺序整行写一遍即可。
2. **隐藏**：redb 新表 `emoji_hidden(词, emoji)`，查询期在 `plan_emoji_insertions` 的
   `seen` 集合里预先装入该词被隐藏的 emoji。不进缓存文件（缓存是全局的、隐藏是用户的）。
3. **菜单**：emoji 候选的右键只给两项——「不再显示此 emoji」（写 2）与「编辑 emoji 词表」
   （打开 1 的文件，同字符类的外部编辑对话框那套）。**不复用**置顶/前移/后移：那些是
   跨候选的位置语义，emoji 只在自己词的小列表里有序，两套语义混在一个菜单里只会误导。

⛔ 不走的路：把 emoji 写成 shadow 规则（`(schema, code, text)`）。它按输入码落键，同一个
词在五笔和拼音下码不同，规则得写两份；宿主词一换 emoji 就成孤儿。emoji 的归属是
**词**，不是码。
...
| 症状 | 根因 | 修法 | 落点 |
|---|---|---|---|
| 含 emoji 的候选行比汉字行高，候选窗随之抖动 | 行高取 DirectWrite 自然度量＝行内所有回退字体里最大的 line metrics；雅黑 +5%、宋体 +17% | 每个 layout 钉 UNIFORM 行距，行高与基线都取基准字族自身的自然值（探针「中a」的 line metrics，按 (字号, 字重, 字族) 缓存；基线不可写成行高的固定比例，各字族 0.78～0.81 不等） | `wind-ui/text/dwrite.rs` `line_height_for`；macOS `coretext.rs` 同步改为只看基准字体度量 |
