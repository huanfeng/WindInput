...
### 5.7 续篇：候选跟随输入大小写（2026-09-09）

用户诉求：「英文方案和临时英文，在输入时并没有按照用户输入的大小写进行处理」。
打 `Hi` 希望候选是 `Hill`，打 `WoW` 希望是 `WoW` —— 但「如果字母不对应则不能这样处理」。

#### 规则：逐位投影，**单向**

对每条词库候选，逐位比对输入与候选的公共前缀：
...
**为什么小写不投影**：词库里 `China` / `iPhone` / `I` 的大写是词本身的一部分。双向投影会
让打 `chi` 的用户再也打不出 `China`，而他并没有表达「我要小写」——小写只是默认击键形态，
按 Shift 才是一个显式动作。真实词库里的证据：`Hi-Fi`，见 `english_case_follow.rs` 的
`lowercase_input_leaves_dict_case_alone`。

**为什么不对应要整条放弃**：英文引擎是大小写不敏感的**前缀**匹配，正常词库候选必然逐位
对应；对不上的只会是短语 / 命令 / `$` 展开这类没有前缀关系的条目。逐位跳过会产出既非词库
形态、也非用户所打的杂合体（`WoWzy`），整条放弃恰好把它们原样留下。

#### ★★ 这是一次**有记录的否决被推翻**

提交 `ed4eb88b` 曾删掉 `adapt_en_case`，理由写在 `handle_temp.rs`：「临英由 Shift+字母
进入，缓冲首字母恒大写，旧适配便把整列候选强制套成 `Hello`/`Help`，而词库 86% 的词本是
小写」。本次用户（2026-09-09）明知这段历史仍拍板重做，区别不在观感而在规则：

- 旧实现是**整串套形**——不管词库原文长什么样，一律按输入形态重写整条；
- 本次是**逐位投影且单向**——只覆盖用户按了 Shift 的那几位，词库自带的大写一律保留。

⇒ 凡是重做一个被删过的功能，**新旧规则的差别必须写在代码注释里**，否则下一个人会照着
当年的理由再删一次。

#### 三条实施纪律（都属于「改错了也不报错」）

1. **投影必须排在词频重排与候选调整之后**。两者都以候选 `text` 为键（`record_freq(schema,
   code, text)`），而读端排在投影之前、看到的恒是词库原文。先投影 ⇒ 写 `Hill`、读 `hill`，
   两端永不相交，**英文词频与置顶整体静默失效**。被改写的候选把原文留在
   `Candidate::case_source`，记账走 `freq_text()` / `record_selection_cased_in`
   （词频取原文、上屏历史取实际上屏形态）。
2. **去重必须排在投影之后**。投影把词库的 `hi` 改写成 `Hi`，与头部原文候选撞车；全大写档
   更会把三条大小写变形塌成一条。
3. **英文方案侧的输入形态取自影子串 `input_buffer_cased`**。`input_buffer` 恒为全小写
   （见 `State::input_buffer`），拿它取形态的话整个功能在英文方案下一条也不会投影，且毫无
   报错。顺带修掉一个既存缺陷：此前英文方案下打 `WoW`，头部「原文候选」显示的是 `wow`。

#### 5.7.1 英文方案下 Shift+字母不再进临时英文

`handle_lifecycle.rs` 那道临英门此前没问过当前方案，于是**在英文方案里**按 Shift+H 会被
换进临英——另一套配置、另一个上屏出口、另一份候选开关，只因两者词库桶恰好共用 `english`
才不太看得出来。加 `!active_is_english()` 后按键落主路字母臂，那里本就把大写存进影子串、
缓冲仍为小写，正是「英文方案 + 首字母大写的输入态」。
