...
> 状态：**P0–P3 已全部实施并合入 main**（2026-08-30，7 个提交）。真机验证未做——
> 实施与验证全程使用伪造的层目录。
...
> 面向定制者的完整指南已落在文档站：`WindInputDocs` 的
> `content/docs/guides/custom-edition.mdx`（站内路径 `/docs/guides/custom-edition`）。
> 下面这几条是**判据性**的原文，指南是它的面向用户改写——改动其一时对照另一处。
