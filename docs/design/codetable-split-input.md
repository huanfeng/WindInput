...
写成 `#[ignore]` 的探针（同整句的真实词库探针），**不进 CI**（依赖用户自带词库）。

⚠️ 探针语料**不能拿词库条目回测**——那恒能切分，是整句那篇 §8 点名的假绿。
...
**测试 17 条**：`codetable/engine.rs` 13 条（基本盘 / 字段约定 / preedit / 关闭零变化 /
...
**仍待做**：§7 的论证复核探针；真机验证（清单见 §8.1）；混输下的档位归属。
