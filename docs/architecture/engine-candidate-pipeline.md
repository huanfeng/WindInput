...
- **`convert` 是合并而非劫持**：含分词符时三路合并，序为 原路径 → **词组分段** →
  **剥离分词符再查一次**，并按 `text` 跨三路去重。词库里 57 条 code 自带撇号
  （`you're` / `o'clock`），劫持式实现会让它们在打全码时失踪；而剥离那路接住的是
  「同一个词在别的词库里编码不含撇号」（靶机词库 `o'clock → oclock`）与拼接式编码的词组
  （`mac'os` → `macOS Tahoe`，分段路径对它无能为力，`macOS` 在 text 里是一个词）。
- **剥离查询的命中须过闸**：只收 `text` 含分词符或多词的条目
  （`english.rs::stripped_hit_is_relevant`）。其余命中一律是普通单词的前缀补全，它们
  **不打分词符时本来就在**，打了还出就会按词库权重铺满 `max_candidates`——实测 `p'o` 被剥成
  `po` 后整屏是 pocket / pod / poem，分段候选一条都挤不进来。同理去重也必须跨三路：
  `mac'os` 下 `Mac OS X` 分段与剥离双命中。重复本身到不了用户眼前（协调器那道按 text 的
  通用去重会接住），但 `truncate(max_candidates)` 在引擎内——每条重复白占一个名额，挤掉的
  词组协调器再去重也变不回来。故该条的护栏在引擎单测，e2e 层按整表断言实测恒绿。
- **同权重时分段候选压过剥离候选**（`natural_order` 续号序）：按下分词符就是在表达
...
| 截断保底 | 取回的英文**全部保活**（`merge()` 先腾座后追加）。混输里英文无配额（见 §7 档位表下的说明），纯拼音下若照搬，拼音一次吐满 `max_candidates` 会把英文整片截掉 ⇒ 开关等于没做 |
...
#### 6.1.1 英文只收精确命中（两条路共用的收窄）

`english_merge::lookup` 与 `MixedEngine::english_display_candidates` 都**只保留
`code == 小写化输入` 的条目**，前缀扩展整片丢弃。实测现场：全拼 `hen` 曾混进
hen / hence / henceforth / Henderson / Hendrix，混输 `github` 曾混进 8 条 GitHub 开头的
长词组。前缀扩展在中英混合输入里没有使用场景——想打某个英文词就继续敲完它。

收窄后英文天然只剩 1~2 条，**按比例分席的配额机制随之删除**（原 `seats_for`）。

⚠️ **上屏否决的判据不跟着收窄**：`english_merge::has_any` 与
`MixedEngine::english_candidates`（`auto_commit_block_on_english` 用）仍按前缀判。
用户打到 `gith` 时精确命中尚不存在，否决若也要求精确，五笔满码就会在第 4 键把中文顶上屏。
一处收窄、一处保留，是这条改动里最容易被后人「顺手统一」掉的地方。

#### 6.1.2 英文候选的位次由协调器决定

英文候选**排第几**不在引擎层决定。`is_exact_code` 在两处取数时即被清除——那是**码表域**
标志（语义为「码 == 输入的完全匹配」，服务码表精确档），而拼音引擎从不设它；跨来源带进
拼音候选列表会让英文在 `candidate_display_order` 的 `cmp_exact_first`（位置在 `by_weight`
**之前**）无条件压过全部中文。真机现象即「打 `hen`，权重 500 的英文排在权重 86016 的
『很』之前」——英文不是赢了权重，是赢在一个拼音压根没参赛的键上。

位次统一由协调器 `place_english_after_common_exact` 在**所有排序（含自动调频）之后**
定位：插在「开头那段连续的 `source_tier <= 1` 候选」之后，即中文常用精确解之后、其余
候选之前。采用后置定位而非新增比较键，理由见该函数文档（比较键会扰动非英文候选之间的
既有次序，而只对英文表态的比较器不是全序）。
...
##### 补充平面按**平面**整体纳入，不逐块列举（issue #83）

`is_han` 的补充平面那一项是 `0x20000..=0x3FFFF`——**整个平面 2（SIP）与平面 3（TIP）**。
这两个平面专用于表意文字，扩展 B–J 与兼容汉字补充全在其中，将来的扩展 K/L 亦然。

原先此处逐块列举到 `0x323AF`（扩展 H 末尾）。Unicode 17 新增的扩展 J 从 `0x323B0` 起，
**只差一个码位**就落到判定域外 ⇒ 恒判常用 ⇒ 常用字档与智能档都放行：虎码跟进 Unicode 17
后，用户的常用字候选里冒出一批无字形的扩 J 生僻字，且在「加入常用字」时被告知「不是汉字」。
同一份列举还漏掉了扩展 I（`2EBF0–2EE5F`）与兼容汉字补充（`2F800–2FA1F`）。

**可复用的形状：逐块列举一份仍在增长的 Unicode 区间，保证每升一版就静默漏一次。** 漏的
方向还恰好是「放行」——不报错、不崩溃，只是过滤悄悄失效。BMP 侧仍需逐块（汉字块之间夹着
假名、注音、标点），但 BMP 已排满、不会再新增汉字块。同一病灶在
`handle_addword.rs`（造词素材域，曾停在扩展 H 且漏扩展 I）与 `gen_dict/entry.rs` 的
`CJK_RANGES`（曾停在扩展 E，`require_cjk` 会把扩展 F 及以后整批丢弃）各有一份，已一并按
平面兜底。

##### 用户覆盖优先于整套作用域，写端放开到全字符（issue #83）

`overrides` 里有这个字符，就无条件照它说的办，**与它是不是汉字无关**；上面整套作用域判断
只对**没被表过态**的字符生效。写端准入相应从 `is_common_scope` 换成 `is_markable`
（空白与控制字符外全放行）。

动机来自用户反馈：字根、间架结构符（`⿰⿱`，U+2FF0–U+2FFF）、注音、假名这些「平常用不到
但还是会显示」的符号，无一落在 `is_common_scope` 内，因而**既滤不掉也加不进常用字表**——
用户的原话是「都是应该通过表来调整，而不是定死了一些东西，改不了」。

三条必须成套的约束，缺一条就会出现静默失效：

1. **读端先放开，写端才能放开**。顺序反了，域外覆盖会存进库里却永不被查询：设了、存了、
   毫无作用，且全程无报错。`markable_char_takes_effect` 钉着这条。
2. **`is_base_common` 对域外字符返回 `true`（常用）**，而不是「查不到就是生僻」。没有覆盖
   的域外字符事实上是被放行的，那就是它们的默认待遇。若返回 `false`，用户把 `、` 设成生僻
   会被 `apply_common_target` 判成「与默认同向」⇒ 删覆盖 ⇒ **设了没有任何反应**。
   `list_all` 报给界面的默认判定同理，不能写死 `false`。
3. **按字判与按串判必须一致**：`is_char_common(c) == is_string_common(&c.to_string())`
   （`char_and_string_judgements_agree` 钉着）。两条路对同一个 `、` 若给出相反答案，词库
   管理页会显示「常用」而候选窗里它已被滤掉，两边都不报错。

未被表过态的字符表现完全不变，故对老用户零回归。「切到 `gb18030` 档就能全部看到」这条
逃生口依旧成立——用户隐藏某类符号后仍需要临时打出它时走这条路。

##### 覆盖表的键是一个**字素簇**，不是一个 `char`（issue #83 续）

用户判据：**「只看显示人是无法分辨的」**。屏幕上一个图形，码位数可以差很多——
`⚽️`=2、`👍🏻`=2、`👨‍👩‍👧`=5（ZWJ 序列）、`🇨🇳`=2（区域指示符对）、`1️⃣`=3（keycap）。
按 `chars().count() == 1` 判「是不是一个字」，这些候选的右键里根本没有「设为生僻字」。

写端准入因此是 `single_markable_char`（UAX #29 字素簇，`unicode-segmentation`），
覆盖表的键随之从 `char` 变成 `String`。**存储层的键本就是 `&str`，不涉及数据迁移**；
JSONL 的字段名保持 `ch`，老备份照旧读得回来。

⛔ 中途曾改成「跳过修饰码位、数基础字符」——`⚽️`/`👍🏻` 通了，ZWJ 序列、国旗、keycap
照旧被拒。**那仍是自己列举 Unicode 规则，与 `is_han` 逐块列举同一个病**。别再退回去。

- **只登记组合、不牵连成员**：关掉 `👨‍👩‍👧` 不会连 `👨` 一起关。
- **热路径闸门**：`is_string_common` 对每个候选都要跑，字素簇分割比逐 `char` 贵，故
  `CommonChars` 存一个装载期算好的 `has_multi_char_keys`——没有多码位键时走原来的逐
  `char` 路径，开销与改动前一致。实测（`tests/perf_common_chars.rs`，release）：
  闸门关 89 ns/次、开 124 ns/次，一屏十来个候选即便开着也只多约 0.4 微秒。
- **导入导出：文本段与数组段并存**。单字符拼进 `common`/`rare` 文本串（与旧版同形），
  多码位另起 `common_seq`/`rare_seq`，**数组段为空时整段不输出**——从不登记 emoji 序列
  的用户，文件里根本不会出现它。
  ★ 分段判据是**往返等价**而非「数码位」：`🇨` 与 `🇳` 各自都是单码位，拼进同一个字符串
  却会合成国旗 `🇨🇳`，读回来少一条、多一条本不存在的（变体选择符、组合附加符号同理）。
  按码位数判就得先列举「哪些单码位危险」——又是在列举 Unicode 规则；往返自检不必知道
  规则是什么，接上去、切回来、看是不是原样即可。
