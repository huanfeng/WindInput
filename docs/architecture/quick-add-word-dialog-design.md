...
- 空的一侧照样切得过去，正文换成该侧空态：`无最近输入` / `剪贴板无可用内容`
  （后者的注释交代准入条件「需要单行文本」——不合规与真的没内容在界面上是同一个可见状态，
  不说清用户会以为复制没生效）。
- 「切不切得动」不再是一个需要判断的问题，`toggle_add_word_source` 无守卫。

### 其余约定

- **剪贴板不可用**＝trim 后为空 / 含换行。两条守卫与命令栏 `dict.add` 的
  `sanitize_dict_add_text` 同源，但出口是**静默降级**而非报错。
  **超长不算不可用**，取前 `ADD_WORD_MAX_LEN` 字（见下）。
...
真机验证（2026-09-03，Windows）已通过：`Ctrl+=` → `Tab` 两侧对称切换（含空态）、`↑↓` 裁剪
方向、无最近输入时的自动落点、`Esc` 退出；`Ctrl+Shift+=` 在剪贴板有词/为空两种情形下的预填
来源；`wind-setting --add-word` 裸启动补齐方案与词条。

同一轮真机反馈修掉的三件事见上面两节（上限 20→10、剪贴板惰性读取、超长截断）。
⚠️ macOS 侧 `Tab` 转发仍未验证——`InputController.swift` 没有针对 Tab 的过滤，理论上与 `↑↓`
同路，但没有证据。
...
### 剪贴板超长：截断，不是作废（2026-09-03）

原先超过 `ADD_WORD_MAX_LEN` 就整段当作「无可用内容」，于是复制一句话按 `Tab` 切过去只看到
空态——可用户想要的那个词往往就在开头。现改为**取前 N 字**，与剪贴板来源「取开头 N 字、
`↓` 砍尾」的裁剪方向一致：给出的正是他按 `↓` 之前会看到的东西。

设置端自己读的那份剪贴板同样要截断，但**上限由 core 经 `dict.addWordContext` 的 `maxLen`
交代**——wind-setting 够不着 core 的常量（那边的 crate 在设置仓是 dev-dependency），自己猜
一个数迟早分叉，而分叉的表现是「填得进去、加不进来」：词条填了十几字，「生成编码」出不来、
「确定」被 `check_derivable_word` 拒掉。core 没给 `maxLen`（旧版本 / 没起来）时**不截断**，
猜一个数比不截断更糟。

### 剪贴板惰性读取：修「Ctrl+= 有时明显卡顿」（2026-09-03）

`clipboard_get_text` 是阻塞版（cached 版会给出陈旧内容，加词是执行路径不能用），打不开时
最坏 sleep 重试 **40ms**，而这发生在**按键线程**上。原先进入加词模式时无条件预读一次 ⇒
那 40ms 摊在每一次 `Ctrl+=` 上，而多数人进来是要加刚打的字、根本不按 `Tab`。

现在 `State::add_word_clip` 是 `Option<Vec<char>>`：`None` = 本轮没读过。只有两处会真读——
用户按 `Tab` 切过来（他主动要剪贴板，那一次等待是他要的），以及最近上屏为空时的自动落点
判断（那时本来也没别的可干，且短路求值让「有最近上屏」这条常见路径整个不求值）。

⚠️ 「还没读」与「读了是空」必须分得开，否则每次问「剪贴板有没有东西」都会再读一次。

⚠️ 多读一次不报任何错，只是让用户觉得卡 ⇒ 守门只能靠计数：测试里的 `MockClip` 记下被读了
几次，`entering_with_recent_input_never_reads_clipboard` 断言常见路径**一次都不读**。
