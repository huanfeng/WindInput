...
7.  复制词库文件（pinyin/、wubi86/、common_chars.txt、system.phrases.toml）
8.  复制 schema 配置文件（data/schemas/*.schema.yaml）
9.  复制默认配置文件（data/config.toml）
...
# 4. 验证默认配置已复制
Test-Path "$env:ProgramFiles\WindInput\data\config.toml"
...
- `../build/data/` - 数据目录（dict/、schemas/、config.toml、system.phrases.toml、themes/）
- `nsis/` - NSIS 脚本文件
